...
@microweb_app.route('/signup/v3', methods=['POST'])
def signup_v3():
    db_conn = sqlite3.connect(db_name)
    c = db_conn.cursor()
    sql_statement = "CREATE TABLE IF NOT EXISTS USER_SALT (USERNAME TEXT PRIMARY KEY NOT NULL, HASH TEXT NOT NULL); "
    c.execute(sql_statement)
    db_conn.commit()
    try:
        salt_value=str(os.urandom(32))
        passw=request.form['password'] 
        salt_passw=salt_value+passw    
        hash_value = hashlib.sha256(request.form['password'].encode()).hexdigest()
        sql_statement = "INSERT INTO USER_SALT VALUES ('{0}' , '{1}' ) ".format(request.form['username'], hash_value,salt_value)
        c.execute(sql_statement)
        db_conn.commit()
    except sqlite3.IntegrityError:
        return "Username has been registered\n"
    print('username: ' , request.form['username'], ' password: ', request.form['password'], ' salt_passw: ', salt_passw)
    return "Secure signup succeeded\n"
...
if __name__ == "__main__":
    microweb_app.run(debug=True,host="0.0.0.0", port=8000, ssl_context='adhoc')